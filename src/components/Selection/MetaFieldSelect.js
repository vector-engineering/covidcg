...
const MetaFieldSelect = observer(() => {
  ...
  const setSelected = (field, options) => {
    ...
    let selectedFields = Object.assign({}, configStore.selectedMetadataFields);
    selectedFields[field] = options;

    configStore.updateSelectedMetadataFields(field, options);
  };
  ...
  //   configStore.updateAgeRange(ageRange);
  // };
    ...
    fieldSelects.push(
      <SelectContainer key={field + '_metadata_select'}>
        ...
        <MultiSelect
          ...
          value={configStore.selectedMetadataFields[field]}
